...
/* actuators */
    ...
    dof2MotorPtr->ConfigForwardSoftLimitEnable(true);
  	...
  	dof2MotorPtr->ConfigForwardSoftLimitThreshold(60.0 * (4096.0/360.0));
    ...
    dof3MotorPtr->ConfigFactoryDefault();
 	...
 	dof3MotorPtr->ConfigSelectedFeedbackSensor(FeedbackDevice::SensorDifference, 0);
  	...
  	dof3MotorPtr->ConfigForwardSoftLimitThreshold(180.0 * (4096.0/360.0));
...
namespace bartender_control {
  ...
  void BartenderHWInterface::init() {
    ...
    joint_position_command_[0] = d2r(0.0);
    joint_position_command_[1] = d2r(0.0);
    joint_position_command_[2] = d2r(0.0);
  ...
  void BartenderHWInterface::write(ros::Duration& elapsed_time) {
    ...
    if (enabled) {
      ...
      dof1MotorPtr->Set(ControlMode::Position, r2d(joint_position_command_[0]) * (4096.0/360.0));
	  double mass_x = 12 * cos (d2r(270 + r2d(joint_position_[1]) - (r2d(joint_position_[2]) + 60.0))) + 19 * cos (d2r(90 + r2d(joint_position_[1])));
	  double mass_y = 12 * sin (d2r(270 + r2d(joint_position_[1]) - (r2d(joint_position_[2]) + 60.0))) + 19 * sin (d2r(90 + r2d(joint_position_[1])));
	  double mass_angle = atan2(abs(mass_y), mass_x);
	  double ff2 = kG2 * cos(mass_angle);
      dof2MotorPtr->Set(ControlMode::Position, r2d(joint_position_command_[1]) * (4096.0/360.0), DemandType::DemandType_ArbitraryFeedForward, ff2 + copysign(kS2, dof2MotorPtr->GetClosedLoopError()));
      double ff3 = kG3 * sin(d2r(r2d(joint_position_[2]) + 60.0));
      dof3MotorPtr->Set(ControlMode::Position, (r2d(joint_position_command_[2]) + 60.0) * (4096.0/360.0), DemandType::DemandType_ArbitraryFeedForward, ff3 + copysign(kS3, dof3MotorPtr->GetClosedLoopError()));
    }
